...
fn main() {
    let data = Array::range(1., 15., 1.).into_shape((14, 1)).unwrap();
    let label = Array::range(1., 15., 1.).into_shape((14, 1)).unwrap();
    let mut lin = Linear::new(data, label);
    lin.train(0.01, 3);
    let data = Array::range(11., 25., 1.).into_shape((14, 1)).unwrap();
